...
- **多次复检判定**：每个账号会进行多轮探测，降低单次抖动误判。
- **复检与未扫描状态**：初始扫描后所有账号默认为 `未扫描`，再对 `未知`/`未扫描` 账号执行复检，帮助用户区分真正的异常与探测不足。
- **结果分类**：最终结果为 `正常` / `失败` / `未知`。
- **结果筛选**：支持按结果筛选（全部 / 正常 / 失败 / 未知 / 未扫描）。
- **批量删除**：仅对“失败（确认401）”账号执行批量删除。
- **结果检索**：支持搜索（防抖）与分页展示。
...
本节说明系统如何从“单次探测结果”得到最终判定，帮助理解为何会出现波动与超时误判。
...
- `401` 次数 >= 2：判定为 **失败（确认401）**
- 非 `401` 次数 >= 2：判定为 **正常**
- 超时、网络错误、缺少 `status_code`、证据不足：判定为 **未知**

> 这意味着：超时不会直接进入“失败可删除”，会先落入“未知”等待下一轮观察。
